#
# Model Spec:
#	y_{jt} = p0_{j} * (1 - (alpha_{j}/(alpha_{j} + t))^r_{j})
#
...
simExpGammaTrial <- function(sample_size, horizon, simPars, seed = 1234) {
	require(data.table)
	p0 <- invlogit(rnorm(sample_size, mean = simPars$p0bar, sd = sqrt(simPars$sigmasq_p0)))
	...
	for (i in seq(sample_size)) {
		y <- p0[i] * (1 - (alpha[i] / (alpha[i] + time))^r[i])
...
### GIBBS SAMPLING NOT YET IMPLEMENTED ####
